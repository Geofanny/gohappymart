<?php

return [
    ''        => 'Home',              
    'home'    => 'Home',              
    'coba'    => 'CobaControllers',   
    'nyoba'    => 'NyobaController',
    'keranjang' => 'KeranjangController',
    'tentang' => 'TentangController',
    'akun'      => 'AkunController',
    'admin-produk'              => 'AdminProductController',
    'admin-produk/create'       => 'AdminProductController@create',
    'admin-produk/store'        => 'AdminProductController@store',
    'admin-produk/edit'         => 'AdminProductController@edit',
    'admin-produk/update'       => 'AdminProductController@update',
    'admin-produk/delete'       => 'AdminProductController@delete',
    'admin-produk/show'         => 'AdminProductController@show',
];
