...
@section('content')
    ...
    <div class="col-sm-12 mt-3">
        <div class="d-flex mb-3">
            <button id="tabFlashSale" class="btn active me-2 w-25">Big Sale</button>
            <button id="tabProduk" class="btn w-25">Produk</button>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div id="contentFlashSale">
                    <form action="{{ route('bigsale.simpan') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id_promo" id="id_promo" value="{{ $bigsale->id_promo ?? '' }}">
                        ...
                        <div class="row mb-2">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="nama_promo" id="editNamaPromo" class="form-control"
                                    required placeholder="Contoh: 11.11 Big Sale Seru" value="{{ old('jumlah_diskon', $bigsale->nama_promo ?? '') }}">
                            </div>
                            {{-- 💰 Jumlah Diskon --}}
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="jumlah_diskon" class="form-control"
                                    ...
                                    value="{{ old('jumlah_diskon', $bigsale->nilai_diskon ?? '') }}" required>
                            </div>
                        </div>
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="form-check form-check-inline">
                                    ...
                                    {{ old('status', $bigsale->status ?? '') == 'Aktif' ? 'checked' : '' }} value="Aktif">
                                    <label class="form-check-label" for="statusAktif">Aktif</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status" id="statusNonaktif"
                                    {{ old('status', $bigsale->status ?? '') == 'Nonaktif' ? 'checked' : '' }} value="Nonaktif">
                        ...
                        {{-- 🗓️ Tanggal & Waktu --}}
                        @php
                        $mulai = isset($bigsale) ? explode(' ', $bigsale->tgl_mulai) : [null, null];
                        $selesai = isset($bigsale) ? explode(' ', $bigsale->tgl_selesai) : [null, null];
                    @endphp
                        ...
                        <div class="row mt-2">
                            <div class="col-12">
                                <button type="submit" id="btnSimpanFlashSale" class="btn btn-primary w-100 py-2 fw-bold">
                                    ...
                                    {{ isset($bigsale) ? 'Perbarui Big Sale' : 'Simpan Big Sale' }}
                                </button>
                            </div>
                        </div>
                    </form>
                ...
                <div id="contentProduk" style="display: none;">
                    <div class="dt-responsive">
                        <table id="dom-jqry" class="table table-striped table-bordered nowrap">
                            ...
                            <tbody>
                                @foreach ($produkBigsale as $promo)
                                    @foreach ($promo->produks as $produk)
                                        @php
                                            // Hitung harga setelah diskon
                                            if ($promo->tipe == 'Persen') {
                                                $hargaSetelahDiskon =
                                                    $produk->harga - ($produk->harga * $promo->nilai_diskon) / 100;
                                            } else {
        ...
        $(document).ready(function() {
            ...
            $('.btnHapus').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) window.location.href = '/hapusProdukBigsale/' + id;
                });
            });
            ...
            $('#tableProdukModal').on('click', '.btnPilihProduk', function() {
                ...
                const promoTipe = "{{ $bigsale->tipe ?? '' }}";
                const promoNilai = parseFloat("{{ $bigsale->nilai_diskon ?? 0 }}");
            ...
            $('.modal-footer .btn-simpan').on('click', function() {
                ...
                const promoId = "{{ $bigsale->id_promo ?? '' }}";
